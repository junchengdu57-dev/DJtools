...
(function() {
    ...
    let SR_LOGIN_PROMPTED = false;
    let SR_LOGIN_PROMPTED_TS = 0;
    ...
    async function ensureSagerealSession(updateStatus) {
        ...
        if (!ok) {
            ...
            if (!SR_LOGIN_PROMPTED || (Date.now() - SR_LOGIN_PROMPTED_TS) > 15000) {
...
function calcOT(schedule, clockIn, clockOut) {
            ...
            <div id="manual-modal">
                ...
                <div class="manual-content">
                    ...
                    <ul>
                        ...
                        <li>
                        <strong> 工作量统计修复</strong>
                        <li>
                            查到了“工作量统计点击会打开两次萨瑞登录页”的根因，并做了稳妥修复。
                        </li>
                        </li>
                    </ul>
                    <h3>❤️ V44.5 版本更新</h3>
    ...
    function initWheel() {
        ...
        function renderWheel(mode) {
            ...
            if (currentMode === 'MENU') { SR_LOGIN_PROMPTED = false; }
    ...
    async function queryWorkload() {
        const ready = await ensureSagerealSession((m)=>document.getElementById('status-bar').innerText=m);
        if (!ready) {
            const btn0 = document.getElementById('btn-buy'); if (btn0) { btn0.disabled = false; btn0.innerText = "确认查询"; }
            const sb0 = document.getElementById('status-bar'); if (sb0) sb0.innerText = "请先在新打开的登录页完成登录，然后再点击查询";
            return;
        }
